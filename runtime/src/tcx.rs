...
decl_storage! {
  trait Store for Module<T: Trait> as Tcx {
    ...
    // Debug
    // CqAmount get(cq_amount): BalanceOf<T>;
    // CqInvested get(cq_invested): BalanceOf<T>;
    // CqMin get(cq_min): BalanceOf<T>;
    // CqFactor get(cq_factor): BalanceOf<T>;
    // CqStaked get(cq_staked): BalanceOf<T>;
    // CqQuota1 get(cq_quota1): BalanceOf<T>;
    // CqQuota2 get(cq_quota2): BalanceOf<T>;
    // CqMax1 get(cq_max1): BalanceOf<T>;
    // CqMax2 get(cq_max2): BalanceOf<T>;
  }
}
...
decl_module! {
  ...
  pub struct Module<T: Trait> for enum Call where origin: T::Origin {
    ...
    action_id: T::ActionId) -> Result {
      ...
      // let quota = <BalanceOf<T>>::from(100);
